...
public class TabuleiroMaluco extends JPanel implements AcoesTabuleiro {
	...
	/**
	 * partes do swing sairao na versao final 
	 * e vao para as classes do front end
	 * 
	 */
	...
	public void criarCampo() {
		...
		for (int i = 0; i < tamMapa; i++) {
			if (posicaoBombas.contains(i)) {
				mapa.add(new CelulaMaluca(i, true));
		...
		if (!celula.getBandeira()) {
			...
			if (celula.getTipo() == 3) {
				...
				if(!celula.getMinada()) {
					...
					JOptionPane.showMessageDialog(null, "YOU ARE A FAILURE","U ARE A WASTE OF CARBON", JOptionPane.INFORMATION_MESSAGE);
					System.exit(1);
	...
	public void colocarBandeira(Celula celula) {
		...
		if(minasMarcadas.size() == bombasNoCampo) {
			JOptionPane.showMessageDialog(null, "Parabéns!","Não fez mais que sua obrigação", JOptionPane.DEFAULT_OPTION);
			System.exit(1);
		}
		...
		for(int i = 0; i<updateAlertas.size();i++) {
			if(mapa.get(updateAlertas.get(i)).getTipo() == 2) {
				...
				if(mapa.get(updateAlertas.get(i)).getNumBombasVizinhas() == 1) {
					mapa.set(updateAlertas.get(i), new CelulaVazia(celula.getPosicao()));
					...
					((CelulaAlerta)mapa.get(updateAlertas.get(i))).setNumBombasVizinhas(numAtual-1);
				}
